...
public class PoisonLakeFeature extends Feature<AetherLakeConfiguration> {
    private static final BlockState AIR = Blocks.CAVE_AIR.defaultBlockState();

    public PoisonLakeFeature(Codec<AetherLakeConfiguration> codec) {
        super(codec);
    }
    ...
    private void createBarrier(BlockPos pos, WorldGenLevel level) {
        if(this.mayBarrierReplace(level.getBlockState(pos.north())))
            level.setBlock(pos.north(), DAFeatureStates.AETHER_MUD, 2);
        if(this.mayBarrierReplace(level.getBlockState(pos.south())))
            level.setBlock(pos.south(), DAFeatureStates.AETHER_MUD, 2);
        if(this.mayBarrierReplace(level.getBlockState(pos.east())))
            level.setBlock(pos.east(), DAFeatureStates.AETHER_MUD, 2);
        if(this.mayBarrierReplace(level.getBlockState(pos.west())))
            level.setBlock(pos.west(), DAFeatureStates.AETHER_MUD, 2);
    }

    @SuppressWarnings("deprecation")
    private boolean mayBarrierReplace(BlockState state) {
        if(state.getFluidState().is(DAFluids.POISON_FLUID.get()) || state.getFluidState().is(DAFluids.POISON_FLOWING.get())) {
            return !state.getFluidState().isSource();
        }
        else return canReplaceBlock(state) && !state.isSolid();
    }
